...
library(TCGAutils) #From bioconductor
...
df_fileUUIDtoCaseUUID <- UUIDtoUUID(manifest$id) #bioconductor TCGAutils tool file-->case UUID
...
rownames(HCCseq_ISG.RS) <- HCCseq_ISG.RS[,2]
HCCseq_ISG.RS.tmp <- HCCseq_ISG.RS[,-1]
HCCseq_ISG.RS <- HCCseq_ISG.RS.tmp[,-1]
...
#Remove ensmbl and hgnc columns. make rownames hgnc symbol
rownames(HCCseq_IFNG.GS) <- HCCseq_IFNG.GS[,2]
HCCseq_IFNG.GS.tmp <- HCCseq_IFNG.GS[,-1]
HCCseq_IFNG.GS <- HCCseq_IFNG.GS.tmp[,-1]
```
...
#a few random genes in random samples are skewing the color scale and not centering on 0 so no differences apparent, rescale data to bring white to zero
